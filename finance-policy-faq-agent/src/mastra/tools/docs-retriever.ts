...
export const financeDocsRetriever = createTool({
  id: "financeDocsRetriever",
  description: "Search knowledge/<namespace> for relevant finance policy information, billing procedures, and financial FAQ content.",
  ...
  execute: async (raw: any) => {
    ...
    const namespace = (base?.namespace ?? "finance").toString().trim();
    ...
    if (!(await dirExists(root))) {
      return { results: [], error: `Finance policy namespace '${namespace}' not found at ${root}.` };
    }
    ...
    try {
      ...
      return { results: [], error: `Failed to read finance policy directory: ${e?.message || "unknown error"}` };
    }

    const files = entries.filter(f => /\.(md|mdx|txt)$/i.test(f));
    if (files.length === 0) {
      return { results: [], error: `No finance policy files found in '${namespace}' namespace.` };
    }

    const { phrases, words } = extractTokens(query);
    const tokens = [...phrases, ...words];
    if (tokens.length === 0) {
      return { results: [], sources: [], info: "No searchable finance terms in query." };
    }
    ...
    const hitPromises = files.map(async (f): Promise<Hit | null> => {
      ...
      try {
        ...
        // filename bonus for finance terms
    ...
    if (allHits.length === 0) {
      return { results: [], sources: [], info: "No matching finance policy information found." };
    }
